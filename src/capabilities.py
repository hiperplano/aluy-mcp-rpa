...
valores fixos pra uma máquina específica. Adapta:
...
def _display_is_xrdp(display: str) -> bool:
    """True se o servidor X do `display` (ex.: ':0') é um Xorg do xrdp —
...
def _has_wine() -> bool:
    """Wine instalado? (necessário só p/ automatizar apps Windows via Wine)."""
    import shutil
...
def detect_capabilities(display: str = None) -> dict:
    ...
    # a11y (AT-SPI, LINUX) disponível? — caminho rápido/preciso opcional.
    try:
