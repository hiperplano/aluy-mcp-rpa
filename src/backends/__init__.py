...
def detect_backends(caps: dict = None):
    ...
    # Motor de detecção de capacidades — adapta em vez de fixar p/ uma máquina específica.
    if caps is None:
        try:
            ...
            if is_linux and not caps.get("x_ok", True):
                print(f"[rpa] ERRO: servidor X não acessível em DISPLAY={caps.get('display')} "
                      "— abra um servidor X (ex.: Xvfb) antes. Sem X o motor não opera.", file=sys.stderr)
            ...
            if is_linux and not caps.get("wine", True):
                print("[rpa] INFO: Wine não instalado — apps Windows (via Wine) "
    ...
    # Ação: backend por OS. Linux → Xlib (proveado, XTest/XSendEvent);
