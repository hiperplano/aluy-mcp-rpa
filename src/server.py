...
TOOLS = [
    ...
    Tool(
        ...
        description=("Digita texto no elemento com foco. Use clear=true p/ LIMPAR o campo "
                     ...
                     "alguns apps Wine, que senão concatenam e viram lixo). ATENÇÃO: o teclado pode "
    ...
    Tool(
        ...
        description=("Pressiona tecla ou combo (ex.: 'ctrl+c', 'alt+F4', 'Return', 'Escape', 'F9'). "
                     "ATENÇÃO: atalhos/teclas podem NÃO funcionar em apps Wine e alguns "
...
def _perceive(engine, *, full: bool = False, with_image: bool = False, extra: dict = None):
        ...
        try:
            ...
            if g is not None:
                ...
                if engine._ui_pending_menu and sid:
                    ...
                    # o dropdown (apps padrão, ex.: Notepad); menus CUSTOM não
    ...
    # custom-desenhada, ex.: um canvas/gráfico) ou full=True. Em janela UIA-rica
...
3. AÇÃO: PREFIRA CLIQUES. O mouse (`rpa_click_text`, `rpa_click_at`) funciona em QUALQUER app.
   ...
   em apps Wine e alguns toolkits. Regra: se uma tecla/atalho não mudar a tela,
   NÃO insista no teclado — abra o recurso por clique (botão de toolbar, menu, painel).

4. ESPERE O INESPERADO. Podem surgir diálogos de confirmação/termos (ex.: "Aceitar termos",
   ...
   ATENÇÃO — MODAIS SÃO OUTRA JANELA: um diálogo (ex.: "Salvar como…", "Substituir", "Fonte")
...
def run() -> None:
    """Entry-point SÍNCRONO do console_script `aluy-mcp-rpa` (uvx/pip/pipx).

    O `main()` é async; um console_script (e o `uvx aluy-mcp-rpa`) precisa de uma
    função sem-args síncrona — este wrapper só roda o loop. Mantém o `__main__`
    abaixo funcional para `python -m`/`python src/server.py`.
    """
    asyncio.run(main())
